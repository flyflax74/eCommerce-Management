...
public interface SectionRepository extends JpaRepository<Section, Integer> {
	...
	List<Section> findAllSectionsSortedByOrder();
	...
	Section getSimpleSectionById(Integer id);
	
	@Query("SELECT NEW Section(s.id) FROM Section s ORDER BY s.sectionOrder ASC")
	List<Section> getOnlySectionIDsSortedByOrder();
	
	@Query("UPDATE Section s SET s.sectionOrder = ?1 WHERE s.id = ?2")
	@Modifying
	void updateSectionPosition(int sectionOrder, Integer id);
}
